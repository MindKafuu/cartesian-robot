...
void SM_RxD(unsigned int8 c)
{
    ...
    if (check_data > 12)
    {
        ...
        checksum = (((int)positionX[0] + (int)positionX[1] + (int)positionY[0] + (int)positionY[1]) - ((int)theta + 
                   (int)modeSystem.mode + (int)modeSystem.type + (int)modeSystem.setType)) + 1;
        if (checksum < 0)
            checksum = checksum * -1;
...
void servo(int degree)
{
	...
	float time_servo = (9.4 * degree) + 700; 
